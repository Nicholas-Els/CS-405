...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {
        // If the resulting number is bigger than the integer's maximum range, send out a warning
        // Else, proceed with the increment
        if (result > INT_MAX) {
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        // If the result is smaller than the integer's minumum range, send out a warning
        // else, procreed with the decrement.
        if (result < INT_MIN) {
            std::cout << "Integer Underflowed" << std::endl;
        }
        else {
            result -= decrement;
        }
